...
class ContEnv(gym.Env):
    ...
    def __init__(
        ...
        hmax: int = 100, #(윤진) 오류 나서 기본 값으로 설정해둠
    ...
    def _buy_stock(self, index, action):
        def _do_buy():
            if (
                ...
                available_amount = self.state[0]//((self.state[index + 1]+1.1) * (1 + self.buy_cost_pct[index]))
                #정규화 오류 괜찮?
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.orig_actions_memory.append(actions)
            ...
            self.reward = (end_total_asset - begin_total_asset)#*100 # 리워드 증가 수정부분
    ...
    def save_action_memory(self):
        if len(self.df.tic.unique()) > 1:
            ...
            orig_action_list = self.orig_actions_memory
            df_actions = pd.concat([df_actions, pd.Series(orig_action_list, name='orig actions')], axis=1)
            # df_actions = pd.DataFrame({'date':date_list,'actions':action_list})
        else:
            date_list = self.date_memory[:-1]
            action_list = self.actions_memory
            orig_action_list = self.orig_actions_memory
            df_actions = pd.DataFrame({"date": date_list, "orig actions": orig_action_list, "actions": action_list})
        return df_actions
...
class Disc3Env(ContEnv):
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            self.asset_memory.append(end_total_asset)
            self.date_memory.append(self._get_date())
            self.reward = (end_total_asset - begin_total_asset)#*100 # 리워드 증가
            self.rewards_memory.append(self.reward)
            self.reward = self.reward * self.reward_scaling
            self.state_memory.append(
                self.state
    ...
    def step(self, action):
        ...
        for index in range(self.stock_dim):
            if action in [0, 1]:  # Buy actions
                ...
                num_shares = -1 * self._sell_stock(index, action)
            else: num_shares = 0
        # actions = np.full(actions.size, num_shares)
        self.actions_memory.append(num_shares)
        self.orig_actions_memory.append(action)
...
# MARK: Disc201Env    
class Disc201Env(ContEnv):
    """Stock trading environment with 201 discrete actions"""

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.action_space = spaces.Discrete(201)  # 0~200 actions

    def _sell_stock(self, index, num_shares):
        """Sell a specific number of shares of a stock"""
        current_shares = self.state[index + self.stock_dim + 1]
        if self.state[index + 2 * self.stock_dim + 1] != True and current_shares > 0:
            actual_sell = min(num_shares, current_shares)
            if actual_sell > 0:
                sell_amount = self.state[index + 1] * actual_sell * (1 - self.sell_cost_pct[index])
                self.state[0] += sell_amount
                self.state[index + self.stock_dim + 1] -= actual_sell
                self.cost += self.state[index + 1] * actual_sell * self.sell_cost_pct[index]
                self.trades += 1
                return actual_sell
        return 0

    def _buy_stock(self, index, num_shares):
        """Buy a specific number of shares of a stock"""
        if self.state[index + 2 * self.stock_dim + 1] != True:
            cost_per_share = self.state[index + 1] * (1 + self.buy_cost_pct[index])
            available_cash = self.state[0]
            max_affordable = int(available_cash // cost_per_share)
            actual_buy = min(num_shares, max_affordable)
            if actual_buy > 0:
                buy_amount = cost_per_share * actual_buy
                self.state[0] -= buy_amount
                self.state[index + self.stock_dim + 1] += actual_buy
                self.cost += self.state[index + 1] * actual_buy * self.buy_cost_pct[index]
                self.trades += 1
                return actual_buy
        return 0
    ...
    def step(self, action):
        """
        action: int from 0 to 200
            - 0~99   : Buy (100 - action) shares
            - 100    : Hold
            - 101~200: Sell (action - 100) shares
        """
        self.terminal = self.day >= len(self.df.index.unique()) - 1
        if self.terminal:
            return self._handle_terminal_state()
        ...
        if self.turbulence_threshold is not None and self.turbulence >= self.turbulence_threshold:
            action = 200  # max sell 100 shares
        ...
        for index in range(self.stock_dim):
            if 0 <= action <= 99:
                num_shares = self._buy_stock(index, 100 - action)
            elif 101 <= action <= 200:
                num_shares = -1 * self._sell_stock(index, action - 100)
            else:
                num_shares = 0  # hold
        self.actions_memory.append(num_shares)
        self.orig_actions_memory.append(action)
        ...
        self.data = self.df.iloc[self.day, :]
        if self.turbulence_threshold is not None:
            self.turbulence = self.data[self.risk_indicator_col] if len(self.df.tic.unique()) == 1 else self.data[self.risk_indicator_col].values[0]
